...
object FIntepolator extends MacroStringInterpolator[String] {
  ...
  protected def getListOfExpr(strCtxExpr : Expr[StringContext])(implicit reflect: Reflection): List[Expr[String]] = {
    ...
    strCtxExpr.unseal.underlyingArgument match {
      case Term.Apply(Term.Select(Term.Select(_, "StringContext"), "apply"),  //new too- only stringcontext important and add the ones that have diff formats
        List(Term.Typed(Term.Repeated(strCtxArgTrees, _), TypeTree.Inferred()))) =>
  ...
  override protected def interpolate(strCtxExpr: Expr[StringContext], argsExpr: Expr[Seq[Any]])(implicit reflect: Reflection): Expr[String] = {
    ...
    def checkSizes(format : Int, argument : Int, argPos : Position, strPos : Position) = {
      ...
      if(format == -1) error("there are no parts", strPos) 
    } 
     ...
     * true if there is a width parameter, false otherwise and 
     * true if there is a precision parameter, false otherwise
     */
    def getFormatTypeIndex(s : String, argPos : reflect.Position, noArg : Boolean) : (Int, List[Char], Boolean, Boolean) = {
      var i = 0 //i is the index of the conversion string in the given string s
      var width = false //true if there is a width, false otherwise
      var precision = false
      val l = s.length
    ...
    def getFlags(i : Int, l : Int, s : String) : List[Char] = {
      if(i < l) {
        val curr = s.charAt(i)
        if(isFlag(curr)) curr :: getFlags(i + 1, l, s)
      } 
      Nil
    }
    ...
    if(!parts.isEmpty) {
      if(parts.size == 1 && args.size == 0 && parts.head.size != 0){
        ...
        val (i, flags, width, precision) = getFormatTypeIndex(parts.head, position, true)
        parts.head.charAt(i) match {
          case 'n' if(width) => error("width not allowed", position) //TODO : should only be one position
          case 'n' | '%' => if(precision) error("precision not allowed", position) //TODO : should only be one position

//TODO : add assertCompile or assertNotCompile(string) for negative string
//TODO : for this error _ msg, sourcefil(pos), start, end - assertion that start < end, do not use columns
          case illegal => 
        }
      }
      val zippedd = (parts.tail, args.map(_.unseal), partsExpr.tail).zipped
      for {(part, arg, partExpr) <- zippedd} {
        val (i, flags, width, precision) = getFormatTypeIndex(part, arg.pos, false)
        part.charAt(i) match { 
            ...
            case 'c' | 'C' => {
              ...
              for{flag <- flags ; if (flag != '-')} error("Only '-' allowed for c conversion", partExpr.unseal.pos) //TODO : should only be one position
              if(precision) error("precision not allowed", partExpr.unseal.pos) //TODO : should only be one position
            }
            ...
            case 'd' => {
              ...
              for{flag <- flags} flag match {
                case '#' => error("# not allowed for d conversion", partExpr.unseal.pos) //TODO : should only be one position
                case _ => //OK
              }
              if(precision) error("precision not allowed", partExpr.unseal.pos) //TODO : should only be one position
            }
            ...
            case 'o' | 'x' | 'X' => {
              if (!checkSubtype(arg.tpe, definitions.IntType, definitions.LongType, definitions.ShortType, definitions.ByteType, typeOf[java.math.BigInteger]))
                error("type mismatch;\n found : " + arg.tpe.showCode + "\nrequired : Int\n", arg.pos)
              for{flag <- flags} flag match {
                case '+' | ' ' | '(' if(!checkSubtype(arg.tpe, typeOf[java.math.BigInteger])) => error("only use '" + flag + "' for BigInt conversions to o, x, X", partExpr.unseal.pos) //TODO : should only be one position
                case ',' => error("',' only allowed for d conversion of integral types", partExpr.unseal.pos) //TODO : should only be one position
                case _ => //OK
              }
              if(precision) error("precision not allowed", partExpr.unseal.pos) //TODO : should only be one position
            }
            ...
            case 'a' | 'A' => {
              if (!checkSubtype(arg.tpe, definitions.DoubleType, definitions.FloatType, typeOf[java.math.BigDecimal]))
                error("type mismatch;\n found : " + arg.tpe.showCode + "\nrequired : Double\n", arg.pos)
              for{flag <- flags ; if (flag == ',' || flag == '(')} error("'" + flag + "' not allowed for a, A", partExpr.unseal.pos) //TODO : should only be one position
              if(precision) error("precision not allowed", partExpr.unseal.pos) //TODO : should only be one position
            }
            ...
            case 't' | 'T' => {
              if (!checkSubtype(arg.tpe, definitions.LongType, typeOf[java.util.Calendar], typeOf[java.util.Date]))
                error("type mismatch;\n found : " + arg.tpe.showCode + "\nrequired : Date\n", arg.pos)
              for{flag <- flags ; if flag == '-'} error("Only '-' allowed for date/time conversions", partExpr.unseal.pos) //TODO : should only be one position
              if(precision) error("precision not allowed", partExpr.unseal.pos) //TODO : should only be one position
            }
            ...
            case 'b' | 'B' => {
              if (!checkSubtype(arg.tpe, definitions.BooleanType, definitions.NullType))
                error("type mismatch;\n found : " + arg.tpe.showCode + "\nrequired : Boolean\n", arg.pos)
              for{flag <- flags ; if (flag != '-' || flag != '#')} error("Illegal flag : '" + flag + "'", partExpr.unseal.pos) //TODO : should only be one position
            }
            case 'h' | 'H' | 's' | 'S' => 
              for{flag <- flags ; if (flag != '-' || flag != '#')} error("Illegal flag : '" + flag + "'", partExpr.unseal.pos) //TODO : should only be one position
            
            //Percent and line separator //FIXME : never matched!
            case 'n' if(width) => error("width not allowed", partExpr.unseal.pos) //TODO : should only be one position
            case 'n' | '%' => if(precision) error("precision not allowed", partExpr.unseal.pos) //TODO : should only be one position
            case illegal => 
              val partPos = partExpr.unseal.pos
              error("illegal conversion character '" + illegal + "'", partPos) //TODO : should only be one position
        }
      }
    } 
